# ====================================================
# train_npk_densenet201.py
# Entraînement DenseNet201 + graphique unique loss/accuracy
...
# ====================================================
# DATA AUGMENTATION (avec preprocessing DenseNet201)
# ====================================================
...
def plot_training_history(history1, history2, label, save_dir):
    ...
    plt.savefig(os.path.join(plot_dir, 'densenet201_training.png'), dpi=150)
    plt.close()
...
def train_all_labels(train_df, valid_df):
    print(f"\n{'=' * 60}\n🏆 ENTRAINEMENT POUR TOUS LES LABELS (DenseNet201)\n{'=' * 60}")
    thresholds = {}
...
def evaluate_on_dataset(df, thresholds, dataset_name='test'):
    ...
    results = {
        ...
        'per_label_accuracy': {
            label: float(np.mean(y_true[:, i] == y_pred_binary[:, i]))
            for i, label in enumerate(labels)
        }
    }
...
if __name__ == "__main__":
    print("\n🚀 DÉBUT DE L'ENTRAÎNEMENT AVEC DenseNet201")
