# ====================================================
# train_npk_mobilenetv2.py
# Entraînement MobileNetV2 + graphique unique loss/accuracy
# ====================================================
...
def plot_training_history(history1, history2, label, save_dir):
    ...
    fig.suptitle(f'MobileNetV2 - {label}', fontsize=14)
    ...
    plt.savefig(os.path.join(plot_dir, 'mobilenetv2_training.png'), dpi=150)
    plt.close()
...
def plot_all_metrics_combined(save_dir):
    ...
    - Loss (train/val) pour tous les labels
    - Accuracy (train/val) pour tous les labels
    """
    ...
    for idx, label in enumerate(labels):
        ...
        # Concaténation des epochs (phase1 + phase2)
...
def plot_final_summary(results, save_dir, dataset='test'):
    ...
    plt.ylim(0, 1)
    plt.ylabel('Accuracy')
    plt.title(f'Accuracy par label sur l\'ensemble de {dataset} - MobileNetV2')
...
def train_single_model(train_df, valid_df, label):
    print(f"\n🎯 Entraînement MobileNetV2 pour {label}")
...
def train_all_labels(train_df, valid_df):
    print(f"\n{'='*60}\n🏆 ENTRAINEMENT POUR TOUS LES LABELS (MobileNetV2)\n{'='*60}")
    thresholds = {}
...
if __name__ == "__main__":
    print("\n🚀 DÉBUT DE L'ENTRAÎNEMENT AVEC MobileNetV2")
